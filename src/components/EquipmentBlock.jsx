...
export const EquipmentBlock = ({ title, data, onChange, allowedMains }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="bg-(--card-header) p-4 border-b border-(--border-color) flex justify-between items-center rounded-t-3xl">
      ...
      <div className="p-5 flex flex-col gap-4">
        <div className="flex items-center justify-between bg-(--input-bg) border border-(--input-border) rounded-2xl p-3">
          <span className="text-[11px] text-(--text-muted) font-medium uppercase tracking-wider w-1/3">Set Name</span>
          <select className="w-2/3 bg-transparent border-none outline-none text-sm p-0 text-(--text-main) focus:ring-0 cursor-pointer appearance-none text-right font-medium"
            value={data.set} onChange={(e) => onChange({ ...data, set: e.target.value })}>
            {SET_OPTIONS.map(s => <option key={s} value={s} className="bg-(--bg-color)">{s}</option>)}
          </select>
        </div>

        <div className="flex items-center justify-between bg-(--input-bg) border border-(--input-border) rounded-2xl p-3">
          <span className="text-[11px] text-(--text-muted) font-medium uppercase tracking-wider w-1/4">Main</span>
          <select className="w-1/2 bg-transparent border-none outline-none text-sm p-0 text-(--text-main) focus:ring-0 cursor-pointer appearance-none font-medium"
            value={data.mainStat.type} onChange={(e) => updateMainStat(e.target.value)}>
            {mainStatKeys.map(s => <option key={s} value={s} className="bg-(--bg-color)">{s}</option>)}
          </select>
          <span className="w-1/4 text-right text-(--text-main) font-semibold text-sm">
        ...
        <div className="pt-2">
          ...
          <div className="flex flex-col gap-2">
            {data.substats.map((sub, idx) => {
              ...
              return (
                <div key={idx} className="flex items-center bg-(--input-bg) border border-(--input-border) rounded-xl p-2 hover:bg-(--hover-bg) transition-colors">
                  <div className="w-[50%] pr-1">
                    <select className="w-full bg-transparent border-none outline-none text-[12px] p-1 text-(--text-main) focus:ring-0 cursor-pointer appearance-none font-medium"
                      value={sub.type} onChange={(e) => updateSubstatType(idx, e.target.value)}>
                      {Object.keys(SUBSTAT_BASES).map(s => (
                        <option key={s} value={s} className="bg-(--bg-color) disabled:opacity-50" disabled={selectedByOthers.includes(s)}>{s}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="w-[25%] flex justify-center">
                    ...
                    <div className="flex items-center bg-(--bg-color) border border-(--border-color) rounded-lg overflow-hidden shadow-sm h-7">

                      {/* ปุ่ม - (ลบ) */}
                      <button
                        onClick={() => updateSubstatRolls(idx, String(sub.rolls - 1))}
                        disabled={sub.rolls <= 0}
                        className="w-6 h-full flex items-center justify-center text-(--text-main) hover:bg-(--hover-bg) active:bg-(--border-color) disabled:opacity-20 transition-all cursor-pointer disabled:cursor-not-allowed"
                      >
                      ...
                      {/* ตัวเลขตรงกลาง */}
                      <div className="w-5 h-full text-center text-[12px] font-bold text-(--text-main) border-x border-(--border-color) bg-(--input-bg) flex items-center justify-center">
                        {sub.rolls}
                      </div>
