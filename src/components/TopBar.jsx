  ...
  return (
    <div className="sticky top-4 md:top-6 z-[120] flex justify-end gap-3 w-full pointer-events-none mb-8">
      <div className="relative pointer-events-auto" ref={presetMenuRef}>
        <button onClick={() => setShowPresetMenu(!showPresetMenu)} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[var(--card-bg)] backdrop-blur-xl border border-[var(--border-color)] shadow-md hover:shadow-lg hover:scale-105 transition-all text-sm font-medium text-[var(--text-main)]">
          ...
          <div className="absolute right-0 top-full mt-3 w-80 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="max-h-60 overflow-y-auto custom-scrollbar p-2 space-y-1">
              {presets.length === 0 ? (
                ...
                presets.map(p => (
                  <div key={p.id} className="flex items-center justify-between group p-2 hover:bg-[var(--hover-bg)] rounded-xl transition-colors cursor-pointer" onClick={() => onLoadPreset(p)}>
            ...
            <div className="p-3 border-t border-[var(--border-color)] bg-[var(--input-bg)] flex gap-2">
              <input type="text" placeholder="Name your setup..." value={presetNameInput} onChange={e => setPresetNameInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSave()} className="flex-1 bg-[var(--bg-color)] border border-[var(--input-border)] rounded-xl px-3 py-2 text-sm text-[var(--text-main)] focus:ring-2 focus:ring-[var(--accent)] outline-none transition-all" />
              <button onClick={handleSave} className="bg-[var(--accent)] text-white px-3 py-2 rounded-xl text-sm font-semibold hover:bg-opacity-90 transition-all active:scale-95">Save</button>
            </div>
          </div>
        )}
      </div>

      <button onClick={toggleDarkMode} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[var(--card-bg)] backdrop-blur-xl border border-[var(--border-color)] shadow-md hover:shadow-lg hover:scale-105 transition-all text-sm font-medium text-[var(--text-main)] pointer-events-auto">
