    ...
    <div className="pt-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center px-2 gap-4 pb-2">
        ...
        <div className="flex bg-[var(--input-bg)] p-1 rounded-xl border border-[var(--border-color)] gap-1">
          <button onClick={() => setEqLayout('row')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${eqLayout === 'row' ? 'bg-[var(--accent)] text-white shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}>1 ROW</button>
          <button onClick={() => setEqLayout('grid')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${eqLayout === 'grid' ? 'bg-[var(--accent)] text-white shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}>2x2 GRID</button>
        </div>
      </div>
