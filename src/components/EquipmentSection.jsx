  ...
  return (
    <div className="pt-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center px-2 gap-4 pb-2">
        ...
        <div className="flex bg-(--input-bg) p-1 rounded-xl border border-(--border-color) gap-1">
          <button onClick={() => setEqLayout('row')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${eqLayout === 'row' ? 'bg-(--accent) text-white shadow-sm' : 'text-(--text-muted) hover:text-(--text-main)'}`}>1 ROW</button>
